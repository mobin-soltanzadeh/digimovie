  ...
  return (
    <li dir={result[0]} className={`FifthSlide ${ currentModal===5 ? "opacity-100 visible w-full p-10" : "opacity-0 invisible w-0 p-0" } flex flex-col justify-center items-center gap-y-6 h-full transition-opacity duration-1000 mini-scroll`} >
      ...
      <div className="flex flex-wrap justify-start items-center gap-x-3 md:gap-x-6 gap-y-2">
        ...
        <a href="https://www.linkedin.com/in/mobin-soltanzadeh" target="_blank" className="text-xs sm:text-sm lg:text-base xl:text-lg text-start text-nowrap border border-gray-100 hover:bg-gray-100 hover:text-zinc-900 px-2 sm:px-3 py-1 rounded-full transition-colors duration-300 cursor-pointer">{result[5]} </a>  {/* linkeding accoumt link */}
      </div>
      ...
      { lang==="en" && <h1 className="text-xs sm:text-sm lg:text-base xl:text-lg text-start">click <a href={enCv} className="text-orange-400 hover:text-orange-500 font-dana-medium hover:font-bold" >here</a> to get my مدئCV</h1> }
