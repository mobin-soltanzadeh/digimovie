...
export default function Comment({ commentCount=10, modal, clickHandler }: Props) {
  return (
    <div className="comments flex flex-col justify-start items-start w-full lg:w-250 xl:w-300 mx-auto px-3 lg:px-0 bg-gray-300 dark:bg-zinc-800 rounded-lg  cursor-pointer" onClick={clickHandler} >
      ...
      <ul className={`comments flex flex-col justify-start items-start gap-y-4 w-11/12 mx-auto ${modal ? "h-[500px] mb-4 overflow-y-scroll" : "h-0 m-0"}  p-0 overflow-hidden transition-[height] duration-500`}> 
        
        { [...Array(commentCount).keys()].map(comment => (
        <li className="flex justify-start items-center gap-x-5 w-full px-4 py-2 bg-gray-100 dark:bg-neutral-900 rounded-lg">
            <div className="flex justify-center items-center gap-x-2">
              <BiLike color="#10b981" size={"25px"} />
              <BiDislike color="#f43f5e" size={"25px"} />
              <div className="w-12 h-12 ml-2 border-2 border-black/50 rounded-2xl rotate-45"> <GoPerson className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 text-orange-500 dark:text-orange-400 -rotate-45" /> </div>
            </div>

            <div className="flex flex-col justify-start items-start">
              <h1 className="text-xl text-orange-500 dark:text-orange-400">{comments[comment].from}</h1>
