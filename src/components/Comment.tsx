...
export default function Comment({ commentCount=10, modal, clickHandler }: Props) {
  return (
    <div className="comments flex flex-col justify-start items-start w-full lg:w-250 xl:w-300 mx-auto px-3 lg:px-0 bg-gray-300 dark:bg-zinc-800 rounded-lg  cursor-pointer" onClick={clickHandler} >
      ...
      <ul className={`comments flex flex-col justify-start items-start gap-y-4 w-11/12 mx-auto ${modal ? "h-[500px] mb-4 overflow-y-scroll [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:rounded-full [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500" : "h-0 m-0"}  p-0 overflow-hidden transition-[height] duration-500`}> 
